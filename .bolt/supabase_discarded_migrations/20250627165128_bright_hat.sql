/*
  ...
  1. New Features
    ...
    - Auto-connect new users to all agents

  2. Mock Data
    - Agent profiles: Sarah, Mike, Alex, Rachel, David
    - Regular users: Jane, Kevin
    - Profile URLs for each agent
    - Sample transactions: balanced exchanges, unbalanced, group sessions

  3. Security
    - RLS policy to allow reading agent profiles
    - Auto-connection trigger for new users
*/
...
-- Create mock users in the auth.users table first, then create profiles
DO $$
DECLARE
  sarah_user_id uuid;
  mike_user_id uuid;
...
BEGIN
  -- Generate user IDs for auth.users table
  sarah_user_id := 'b0000000-0000-0000-0000-000000000001'::uuid;
  mike_user_id := 'b0000000-0000-0000-0000-000000000002'::uuid;
  alex_user_id := 'b0000000-0000-0000-0000-000000000003'::uuid;
  rachel_user_id := 'b0000000-0000-0000-0000-000000000004'::uuid;
  david_user_id := 'b0000000-0000-0000-0000-000000000005'::uuid;
  jane_user_id := 'b0000000-0000-0000-0000-000000000006'::uuid;
  kevin_user_id := 'b0000000-0000-0000-0000-000000000007'::uuid;

  -- Insert into auth.users table (if we have access, otherwise skip)
  BEGIN
  ...
  EXCEPTION
    WHEN insufficient_privilege THEN
      -- If we can't insert into auth.users, we'll create profiles without proper user_id references
      -- This is a fallback for development environments
      sarah_user_id := sarah_profile_id;
      mike_user_id := mike_profile_id;
      alex_user_id := alex_profile_id;
      rachel_user_id := rachel_profile_id;
      david_user_id := david_profile_id;
      jane_user_id := jane_profile_id;
      kevin_user_id := kevin_profile_id;
  END;
