/*
  # Fix database functions and enhance time logging system

  1. Database Functions
    - Drop and recreate get_pending_time_transactions function with correct return type
    - Enhanced create_invitation_with_time_log function
    - Update time transaction status function
    - Improved notification system

  2. New Columns
    - Add notification tracking to invitations table
    - Add indexes for better performance

  3. Security
    - Grant proper permissions to authenticated users
    - Update triggers for enhanced notifications
*/
...
-- Function to get pending time transactions for a user (recreated with correct return type)
...
BEGIN
  ...
  SELECT 
    ...
    -- Can confirm if they are the receiver and didn't log it
    (tt.receiver_id = user_profile_id AND tt.logged_by != user_profile_id) as can_confirm,
    -- Can nudge if they logged it and it's still pending
...
-- Function to nudge a pending transaction
...
BEGIN
  ...
  -- Check if enough time has passed since last nudge (1 hour)
  ...
  result := result || jsonb_build_object('message', 'Nudge sent successfully');
...
-- Update the existing triggers to use the enhanced function
