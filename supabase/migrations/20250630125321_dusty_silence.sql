/*
  # Simple Invitation Flow Without Email

  1. Updates
    - Simplify invitation system to work without external email
    - Create shareable invitation links
    - Add better token validation
    - Improve invitation acceptance flow

  2. Functions
    - create_simple_invitation: Creates invitation with shareable link
    - accept_invitation: Processes invitation acceptance
    - get_invitation_details: Gets invitation info for signup page
*/

-- Update the invitation creation function to be simpler
...
BEGIN
  ...
  result := result || jsonb_build_object(
    ...
    'invite_url', 'PLACEHOLDER_URL/invite/' || invitation_token,
...
-- Allow anonymous users to get invitation details (for signup page)
