/*
  ...
  1. New Tables
    - `profiles` - Core user information and time balances
    - `profile_urls` - External links provided by users (GitHub, portfolio, etc.)
    - `work_samples` - Scraped content from profile URLs displayed as cards
    - `skills` - Master list of skills/expertise areas
    - `profile_skills` - Junction table linking users to their skills
    - `connections` - Network relationships between users
    - `time_transactions` - Core time logging and exchange system
    - `transaction_notifications` - Notification system for time transactions
    - `search_queries` - Track user searches for analytics

  2. Security
    - Enable RLS on all tables
    - Add comprehensive policies for data access control
    - Users can only access their own data and connected users' public data

  3. Features
    - Automatic time balance updates via triggers
    - Notification creation for transaction events
    - Full-text search capabilities
    - Performance indexes for fast queries
    - Initial skills data seeding
*/
...
CREATE INDEX IF NOT EXISTS idx_work_samples_title_trgm ON work_samples USING gin(title gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_work_samples_description_trgm ON work_samples USING gin(description gin_trgm_ops);
