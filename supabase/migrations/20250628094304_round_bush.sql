...
DROP POLICY IF EXISTS "Service role can read pending profiles" ON pending_profiles;
...
DROP POLICY IF EXISTS "Service role can manage profile URLs" ON profile_urls;
...
-- Function to manually test the data transfer logic (without calling trigger)
CREATE OR REPLACE FUNCTION test_data_transfer_logic(test_email text, test_name text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  -- Step 2: Simulate the data transfer logic (what the trigger does)
  SELECT * INTO pending_data
  FROM pending_profiles
  ...
  SELECT COUNT(*) INTO url_count FROM profile_urls WHERE profile_id = new_profile_id;
  
  result := result || jsonb_build_object(
    'step2_profile_created', new_profile_id IS NOT NULL,
    'profile_id', new_profile_id,
    'profile_name', pending_data.full_name,
    'profile_display_name', pending_data.display_name,
    'urls_transferred', url_count,
    'urls_added_during_process', urls_added
  );
  
  -- Clean up test data
  DELETE FROM profile_urls WHERE profile_id = new_profile_id;
...
-- Test the data transfer logic
SELECT test_data_transfer_logic('test-transfer@example.com', 'Test Transfer User') as test_result;

-- Function to check current state of pending profiles
CREATE OR REPLACE FUNCTION check_pending_profiles_state()
RETURNS TABLE(
  id uuid,
  email text,
  full_name text,
